...
{
	IActionMapManager		*myActionMapMan = g_pGame->GetIGameFramework()->GetIActionMapManager();

	myActionMapMan->AddExtraActionListener(this);
}
...
bool						CActor::NetSerialize(TSerialize ser, EEntityAspects aspect, uint8 profile, int pflags) { return (true); }
...
bool						CActor::GetEntityPoolSignature(TSerialize signature) { return (true); }

NetworkAspectType			CActor::GetNetSerializeAspects() { return eEA_All; }

ISerializableInfoPtr		CActor::GetSpawnInfo() { return (NULL); }

void						CActor::SetChannelId(uint16 id) { GetGameObject()->SetChannelId(id); }
...
bool						CActor::DropItem(EntityId itemId, float impulseScale, bool selectNext, bool byDeath) { return (true); }
...
bool						CActor::CanBreakGlass() const { return (false); }

bool						CActor::MustBreakGlass() const { return (false); }
...
int							CActor::GetArmor() const { return (_armor); }

int							CActor::GetMaxArmor() const { return (_maxArmor); }

int							CActor::GetAtt() const { return (_att); }

int							CActor::GetMaxAtt() const { return (_maxAtt); }

float						CActor::GetSpeed() const { return (_speed); }

float						CActor::GetMaxSpeed() const { return (_maxSpeed); }

bool						CActor::IsFallen() const { return (false); }

bool						CActor::IsDead() const { return (false); }

int							CActor::IsGod() { return (0); }

IAnimationGraphState		*CActor::GetAnimationGraphState() { return (NULL); }

EntityId					CActor::GetGrabbedEntityId() const { return (0); }

Quat						CActor::GetViewRotation() const { return (Quat(IDENTITY)); }

bool						CActor::IsFriendlyEntity(EntityId entityId, bool bUsingAIIgnorePlayer) const { return (false); }

Vec3						CActor::GetLocalEyePos() const { return (Vec3(0, 0, 0)); }
...
const char					*CActor::GetActorClassName() const { return ("Actor"); }
