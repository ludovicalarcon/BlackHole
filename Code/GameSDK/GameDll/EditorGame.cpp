...
CEditorGame::CEditorGame() : _pGame(0), _pGameStartup(0)
{}
...
bool						CEditorGame::Init(ISystem *pSystem, IGameToEditorInterface *pEditorInterface)
{
	...
	_pGameStartup = CreateGameStartup();
	_pGame = _pGameStartup->Init(startupParams);

	if (!_pGame)
		return (false);
	_pGame->GetIGameFramework()->InitEditor(pEditorInterface);
	gEnv->bServer = true;
...
{
	return (_pGameStartup->Update(haveFocus, updateFlags));
}

void						CEditorGame::Shutdown()
{
	SetGameMode(false);
	_pGameStartup->Shutdown();
}
...
bool						CEditorGame::BuildEntitySerializationList(XmlNodeRef output) { return (true); }

bool						CEditorGame::SupportsMultiplayerGameRules() { return (false); }
...
IEquipmentSystemInterface	*CEditorGame::GetIEquipmentSystemInterface() { return (nullptr); }

IEntity						*CEditorGame::GetPlayer() { return (nullptr); }

bool						CEditorGame::GetAdditionalMinimapData(XmlNodeRef output) { return (true); }

IFlowSystem					*CEditorGame::GetIFlowSystem() { return (_pGame->GetIGameFramework()->GetIFlowSystem()); }

IGameTokenSystem			*CEditorGame::GetIGameTokenSystem() { return (_pGame->GetIGameFramework()->GetIGameTokenSystem()); }

bool						CEditorGame::SetGameMode(bool bGameMode)
{
	if (gEnv->IsEditor())
		_pGame->EditorResetGame(bGameMode);
	IGameFramework * pGameFramework = _pGame->GetIGameFramework();
	return (true);
}
