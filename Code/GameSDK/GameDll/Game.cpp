...
{
	_pFramework = pFramework;
	return true;
}

bool					CGame::CompleteInit() { return (true); }
...
{
	bool bRun = _pFramework->PreUpdate(true, updateFlags);
	_pFramework->PostUpdate(true, updateFlags);
	return bRun ? 1 : 0;
}
...
IGame::TSaveGameName	CGame::CreateSaveGameName() { return (nullptr); }

const bool				CGame::DoInitialSavegame() const { return (true); }

uint32					CGame::AddGameWarning(const char* stringId, const char* paramMessage,
											  IGameWarningsListener* pListener) { return (0); }
