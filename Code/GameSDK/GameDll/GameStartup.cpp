...
CGameStartup::CGameStartup() :  _pMod(NULL),
								_modRef(&_pMod),
								_modDll(0),
								_frameworkDll(NULL),
								_pFramework(NULL),
...
IGameRef		CGameStartup::Init(SSystemInitParams &startupParams)
{
	...
	if (!InitFramework(startupParams))
		return (0);
	ISystem* pSystem = _pFramework->GetISystem();
	startupParams.pSystem = pSystem;
	IGameRef pOut = Reset(GAME_NAME);
	if (!_pFramework->CompleteInit())
	{
		pOut->Shutdown();
		return (0);
	}
...
IGameRef		CGameStartup::Reset(const char *modName)
{
	ModuleInitISystem(_pFramework->GetISystem(), "CryGame");
	static char pGameBuffer[sizeof(CGame)];
	_pMod = new ((void*)pGameBuffer) CGame();
	if (_pMod && _pMod->Init(_pFramework))
		return (_modRef);
	return (0);
}
...
{
	if (_pMod)
		return (_pMod->Update(haveFocus, updateFlags));
	return 0;
}
...
bool			CGameStartup::GetRestartLevel(char** levelName) { return (false); }

const char		*CGameStartup::GetPatch() const { return (nullptr); }

bool			CGameStartup::GetRestartMod(char* pModName, int nameLenMax) { return (false); }
...
{
	_frameworkDll = CryLoadLibrary(GAME_FRAMEWORK_FILENAME);
	if (!_frameworkDll)
		return (false);

	IGameFramework::TEntryFunction CreateGameFramework = (IGameFramework::TEntryFunction)CryGetProcAddress(_frameworkDll, DLL_INITFUNC_CREATEGAME);
	if (!CreateGameFramework)
		return (false);
	_pFramework = CreateGameFramework();
	if (!_pFramework)
		return (false);
	...
	if (!_pFramework->Init(startupParams))
		return (false);
	ModuleInitISystem(_pFramework->GetISystem(), "CryGame");
	return (true);
}
...
void					CGameStartup::ShutdownFramework()
{
	if (_pFramework)
	{
		_pFramework->Shutdown();
		_pFramework = 0;
		CryFreeLibrary(_pFramework);
	}
}
