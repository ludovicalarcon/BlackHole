...
string CStructInfo::ToString(const void* data, FToString flags, const void* def_data) const
{
	...
	for (int i = 0; i < Vars.size(); i++)
	{
		...
		if (!var.IsInline())
		{
			...
			if (!flags._NamedFields && i > 0)
				str += ",";
				
			string substr = var.ToString(data, FToString(flags).Sub(0), def_data);

			if (flags._SkipDefault && substr.empty())
				continue;

			if (flags._NamedFields)
			{
				if (*str)
					str += ",";
				if (*var.Name)
...
CEnumInfo::CEnumInfo( cstr name, size_t size, size_t align, size_t num_elems, CEnumElem* elems)
	: CTypeInfo(name, size, align), Elems(elems, check_cast<int>(num_elems)),
		MinValue(0), MaxValue(0), bRegular(true), nPrefixLength(0)
{
	...
	if (num_elems)
	{
		...
		for (uint i = 0; i < num_elems; i++)
		{
			...
			// Detect hidden elements, must be at end.
			if (*Elems[i].Name == '_')
			{
				Elems[i].Name++;
				if (nDisplayElems > i)
...
bool CEnumInfo::FromString(void* data, cstr str, FFromString flags) const
{
	...
	for (int i = 0; i < Elems.size(); i++)
	{
		if (istr == ShortName(i) || istr == FullName(i))
